...
def _get_event_time(row, diagnosis_days_col, followup_days_col):
    """
    Get event time from a row of the dataframe.
    """
    if row[diagnosis_days_col] == -1:
        tmp = row[followup_days_col]
    else:
        tmp = max(row[diagnosis_days_col], row[followup_days_col])

    return tmp

def get_concordance_index_from_df(data_df: pd.DataFrame, diagnosis_days_col, followup_days_col, score_columns=None):

    times_day = data_df.apply(lambda row: _get_event_time(row, diagnosis_days_col, followup_days_col), axis=1).values
    times_year = times_day / DAYS_PER_YEAR
    times = np.floor(times_year).astype(int)
    times = np.clip(times, 0, 5)

    # These are coded as: -1 for censored, 0 for negative, +1 for positive.
    # Taking the max will tell us if this person was ever positive.
    true_columns = [col for col in data_df.columns if col.startswith("true_")]
    event_observed = data_df[true_columns].max(axis=1).values.flatten()

    scores_df = data_df
    if score_columns is not None:
        scores_df = data_df[score_columns]

    censoring_dist = get_censoring_dist(times, event_observed)
    c_index = concordance_index(
        event_times=times,
        predicted_scores=scores_df.values,
